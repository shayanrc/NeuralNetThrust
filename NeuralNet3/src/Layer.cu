...
//This does not work: Fixed but not tested
...
void Layer::propagateBack(Layer &layer)
{
	...
	for(int neuronIndex=0;neuronIndex<layer.d_dW.size();neuronIndex++)
		{
			...
			d_WxdW=thrust::device_vector<double>(neurons[neuronIndex].d_weightVector.size());
...
void Layer::modifyweights()
{
	...
	for(int neuronIndex=0;neuronIndex<no_of_neurons;neuronIndex++)
	{
		...
		printVector(neurons[neuronIndex].d_weightVector);
		//thrust::transform(neurons[neuronIndex].d_weightVector.begin(),neurons[neuronIndex].d_weightVector.end(),d_inputs.begin(),neurons[neuronIndex].d_weightVector.begin(),modifyweight_functor<double>(learningRate,d_dW[neuronIndex]));
